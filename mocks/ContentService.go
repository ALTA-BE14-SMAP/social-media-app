...
// GetById provides a mock function with given fields: token
func (_m *ContentService) GetById(token interface{}) ([]content.CoreContent, error) {
	ret := _m.Called(token)

	var r0 []content.CoreContent
	if rf, ok := ret.Get(0).(func(interface{}) []content.CoreContent); ok {
		r0 = rf(token)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]content.CoreContent)
